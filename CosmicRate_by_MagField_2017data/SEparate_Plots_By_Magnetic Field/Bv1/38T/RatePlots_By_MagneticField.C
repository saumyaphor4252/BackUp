void RatePlots_By_MagneticField()
{
  ...
  //TFile *file = new TFile("Prompt2017_Av1.root");
   ...
   int nRunsWithNegativeRuntime=0 ,nRuns_with_this_MagField=0, nRunsWithNegativeRuntime_inThisField=0;
   ...
   for (Long64_t jentry=0; jentry<n;jentry++) 
   {
	 ...
	 nRunsWithNegativeRuntime++;

	 if( abs((Mag_Field-GivenField))>1e-2 ) 
	   {
     ...
     if( abs((Mag_Field-GivenField))>1e-2 ) 
       {
	 std::cout<<"Mag Filed is not equal to "<<Mag_Field<<std::endl;
	 continue;
       }
     
     std::cout<<"Mag Filed is equal to "<<Mag_Field<<std::endl;
   ...
   std::cout<<"xxxxxxxxxxxxxxxxxxxxx  Runs with magnetic field "<<GivenField<<" are "<<nRuns_with_this_MagField<<" xxxxxxxxxxxxxxxxxxxxxx "<<std::endl;
   std::cout<<"Runs with negative time are :"<<nRunsWithNegativeRuntime<<std::endl;
   std::cout<<"Runs with negative time in this field are :"<<nRunsWithNegativeRuntime_inThisField<<std::endl;
   ...
   c.SaveAs("tec_track_ratePM.C");
   c.Clear();
   gSystem->Exec("mv tec_track_ratePM.png Rate_Plots");
   gSystem->Exec("mv tec_track_ratePM.C Rate_Plots");
