void Get_Normalized_Kinematic_Plot(TH1D, TString, int);

void Get_Single_Kinematic_Plot()
{
        
  TFile *file = new TFile("Prompt2017_Av1.root");
  float GivenField_1=3.8112;
  int Color=2; //Red=2 for 4T, Blue =4 for 0T 
   ...
   std::cout<<"   Total Events with field : "<<nEvents_MagField_1<<std::endl;
   std::cout<<"   Total Tracks with field : "<<nTracks_MagField_1<<std::endl;
  
   //++++++++++++++++++++++++++++++++++       Make Directory     ++++++++++++++++++++++++++++++++++++++  
   gSystem->Exec("mkdir -p Kinematic_Plots");
   
   //---------------------------------------------------------------------------------------------------
   Get_Normalized_Kinematic_Plot(h_pt_1, "pt", Color);
   Get_Normalized_Kinematic_Plot(h_eta_1, "eta", Color);
   Get_Normalized_Kinematic_Plot(h_phi_1, "phi", Color);
   Get_Normalized_Kinematic_Plot(h_theta_1,  "theta", Color);
   Get_Normalized_Kinematic_Plot(h_d0_1, "d0", Color);
   Get_Normalized_Kinematic_Plot(h_dz_1, "dz", Color);
   Get_Normalized_Kinematic_Plot(h_chi2_1,  "chi2", Color);
   Get_Normalized_Kinematic_Plot(h_chi2_ndof_1,  "chi2_ndof", Color);
   Get_Normalized_Kinematic_Plot(h_charge_1, "charge", Color);
   
}

void Get_Normalized_Kinematic_Plot(TH1D h1, TString variable, int color)
{
  ...
  entry=leg->AddEntry("h_eta_1__3","3.8 T","lpf");
