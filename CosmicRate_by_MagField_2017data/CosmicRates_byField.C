...
void CosmicRates_byField()
{
   /* 1) Go to a Run
      2) Get the magnetic field for that Run from the "Event" TRee and store it in a temprory variable "Run_MagField"   
   ...
   */
   ...
   bool IsFileExist = file->IsZombie();
   if(IsFileExist)
   {
      cout<<endl<<"====================================================================================================="<<endl;
   ...
   TTree *tree;
   tree = (TTree*)file->Get("cosmicRateAnalyzer/Run"); // For getting Track Rates

   FILE * pFile = fopen ("tracksInfo.txt","w");

   // xxxxxxx Temporary variables to store the tree branches
   double  run_time;
   unsigned int runnum;
