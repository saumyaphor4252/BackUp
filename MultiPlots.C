void MultiPlots()
{
   ...
   Int_t events[5]  = {1000, 10000, 100000};
   ...
   while ((key=(TKey*)next()))
   {
     ...
     for(int i=1000;i<=100000;i=i*10, t++)
      {
	 
	 TString cmd1 = Form("%s>>hnew(%i,%f,%f)",BranchName.Data(),Bins,xMin,xMax);
	 
	 std::cout<<cmd1.Data()<<std::endl;
	 if(i==1000) {	 tree->Draw(cmd1.Data(),"","goff",n,0); }
	 else { tree->Draw(cmd1.Data(),"","goff",i,0); }
	 ...
	 else if(BranchName=="chi2_ndf")
	   h->GetXaxis()->SetTitle("Track #chi^{2} /NDF");
	 else if(BranchName=="chi2_Prob")
	 ...
	 //============== Drawing Legends================
         if(i==1000) { sprintf(legEntry, "%lli", n); }
	 else { sprintf(legEntry, "%i", i); }
